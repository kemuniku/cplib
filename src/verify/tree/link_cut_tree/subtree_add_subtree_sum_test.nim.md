...
data:
  ...
  code: "# verification-helper: PROBLEM https://judge.yosupo.jp/problem/dynamic_tree_subtree_add_subtree_sum\n\
    ...
    \ scanf(formatstr: cstring): cint {.header: \"<stdio.h>\", varargs.}\nproc ii():\
    \ int {.inline.} = discard scanf(\"%lld\", addr result)\n\nlet n = ii()\nlet q\
    \ = ii()\nlet values = newSeqWith(n, (sum: int64(ii()), count: 1))\nlet tree =\
    \ newLazySubtreeLinkCutTreeWith(\n    values,\n    (sum: l.sum + r.sum, count:\
    \ l.count + r.count),\n    (sum: 0'i64, count: 0),\n    (sum: x.sum + f * int64(x.count),\
  ...
  verifiedWith: []
documentation_of: verify/tree/link_cut_tree/subtree_add_subtree_sum_test.nim
layout: document
redirect_from:
- /verify/verify/tree/link_cut_tree/subtree_add_subtree_sum_test.nim
- /verify/verify/tree/link_cut_tree/subtree_add_subtree_sum_test.nim.html
