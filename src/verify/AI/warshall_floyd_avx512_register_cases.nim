...
proc check[T](n: int, inf: T, unit: int, enabled: bool) =
    for mode in 0..2:
        ...
        # 分割前と乱数列を揃えるため、担当外の入力も生成してから飛ばす。
...
proc runRegisterTests*(group: int) =
    randomize(512216)
    for n in [217, 223, 224, 225, 255, 256, 257, 433, 447, 448, 449,
            511, 512, 513, 575, 576, 577]:
        let current = if n <= 257: 0
                      elif n <= 449: 1
                      elif n <= 513: 2
                      elif n <= 576: 3
                      else: 4
        check[int](n, INF64, int(10_000_000_001), group == current)
    for n in [257, 271, 272, 287, 288, 289, 513]:
        check[int32](n, INF32, 10_000, group == 5)
    echo "Hello World"
