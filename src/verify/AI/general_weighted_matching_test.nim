...
proc check(n: int, edges: seq[Edge], expected = -1'i64) =
    ...
    let answer = g.maximum_weight_matching()
    validate(n, edges, answer, want)
    validate(n, edges, sg.maximum_weight_matching(), want)
    doAssert g.maximum_weight_matching() == answer
    doAssert g.edges == before and g.edge_info == infoBefore
    doAssert sg.elist == staticBefore and sg.edge_info == infoBefore

static:
    doAssert not compiles(initWeightedDirectedGraph(2).maximum_weight_matching())
    doAssert not compiles(initWeightedDirectedStaticGraph(2).maximum_weight_matching())
    doAssert not compiles(initUnWeightedUnDirectedGraph(2).maximum_weight_matching())
    doAssert not compiles(initWeightedUnDirectedGraph(2, float).maximum_weight_matching())
    doAssert not compiles(initWeightedUnDirectedGraph(2, uint64).maximum_weight_matching())
...
for n in [30, 70, 120]:
    ...
    validate(n, edges, g.maximum_weight_matching(), 17'i64 * int64(g.maximum_matching().len))

block:
    var g = initWeightedUnDirectedGraph(4, int32)
    g.add_edge(1, 2, 2000000000'i32)
    doAssert g.maximum_weight_matching().weight == 2000000000'i64
    g.add_edge(0, 1, 1500000000'i32)
    g.add_edge(2, 3, 1500000000'i32)
    doAssert g.maximum_weight_matching().weight == 3000000000'i64
block:
    var g = initWeightedUnDirectedStaticGraph(3, int16)
    g.add_edge(0, 1, 13'i16)
    g.add_edge(1, 2, 17'i16)
    g.build()
    doAssert g.maximum_weight_matching().weight == 17
    g.add_edge(2, 0, 21'i16)
    g.build()
    doAssert g.maximum_weight_matching().weight == 21
block:
    var g = initWeightedUnDirectedGraph(2, int8)
    g.add_edge(1, 0, 127'i8)
    doAssert g.maximum_weight_matching() == (127'i64, @[(0, 1)])
