...
data:
  ...
  code: "when not declared CPLIB_CONVOLUTION_CONVOLUTION:\n    const CPLIB_CONVOLUTION_CONVOLUTION*\
    ...
    \ l = (if deg == 1: 1 else: (1 shl (fastLog2(deg - 1) + 1)))\n        f.setLen(l)\n\
    \        g.setLen(l)\n        ntt(f)\n        ntt(g)\n        for i in 0..<f.len:\n\
    \            f[i] *= g[i]\n        intt(f)\n        f.setlen(deg)\n        return\
    ...
    \ M1).int, M2.int)[1].uint\n            i3 = inv_gcd((M1 * M2).int, M3.int)[1].uint\n\
    \        # FIXME: mapit\u3067f1, g1\u3092\u4F5C\u308D\u3046\u3068\u3059\u308B\u3068\
  ...
  - verify/AI/convolution_old_test.nim
documentation_of: cplib/convolution/convolution_old.nim
layout: document
redirect_from:
- /library/cplib/convolution/convolution_old.nim
- /library/cplib/convolution/convolution_old.nim.html
