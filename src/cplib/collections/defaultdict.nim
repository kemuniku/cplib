when not declared CPLIB_COLLECTIONS_DEFAULTDICT:
    ...
    proc `==`*[K, V](src, dst: DefaultDict[K, V]) = src.table == dst.table
    ...
    proc contains*[K, V](d: var DefaultDict[K, V], key: K) = d.table.contains(key)
    ...
    proc pop*[K, V](d: var DefaultDict, key: K, val: var V) = d.table.pop(key, val)
    proc take*[K, V](d: var DefaultDict, key: K, val: var V) = d.table.pop(key, val)
