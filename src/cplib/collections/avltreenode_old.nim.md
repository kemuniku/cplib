...
data:
  ...
  code: "when not declared CPLIB_COLLECTIONS_AVLTREE:\n    const CPLIB_COLLECTIONS_AVLTREE*\
    ...
    \ not l.isNil and l != get_avltree_nilnode[K](): l.p = node\n        node.r =\
    \ r\n        if not r.isNil and r != get_avltree_nilnode[K](): r.p = node\n  \
    ...
    \                return lr\n        node.update\n        return node\n    proc\
    \ rebalance_to_root[K](node: AvlTreeNode[K]): AvlTreeNode[K] =\n        var node\
    \ = node\n        while not node.p.isNil and node.p != get_avltree_nilnode[K]():\n\
...
layout: document
redirect_from:
- /library/cplib/collections/avltreenode_old.nim
- /library/cplib/collections/avltreenode_old.nim.html
