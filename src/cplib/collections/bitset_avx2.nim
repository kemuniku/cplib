...
## 命令数はAVX2経路の主ループの演算部分の目安で、関数全体の命令数やサイクル数ではありません。
...
when not declared CPLIB_COLLECTIONS_BITSET_AVX2:
    ...
    proc initBitSetFromString*(s: string, match: char, N: int): BitSetAvx2 =
    ...
    proc initBitSetFromString*(s: string, match: char): BitSetAvx2 =
        ## 文字列のバイト数と同じ長さの集合を構築します。O(s.len)。
        initBitSetFromString(s, match, s.len)

    proc initBitSetFromString*(s, reference: string, N: int): BitSetAvx2 =
    ...
    proc initBitSetFromString*(s, reference: string): BitSetAvx2 =
        ## 文字列のバイト数と同じ長さの集合を構築します。O(s.len)。
    ...
    proc andInto*(dst: var BitSetAvx2, x, y: BitSetAvx2) =
    ...
    proc orInto*(dst: var BitSetAvx2, x, y: BitSetAvx2) =
        ## 確保済みのdstへx | yを書き込みます。全て同じ長さが必要です。O(ビット数 / 64)。
        ## 領域確保・中間集合は不要で、dstにxやy自身を指定することもできます。
        ## AVX2経路の目安: 256ビットあたり論理演算1命令。ロード・ストアと末尾のマスクは別です。
        when compileOption("boundChecks"):
    ...
    proc xorInto*(dst: var BitSetAvx2, x, y: BitSetAvx2) =
        ## 確保済みのdstへx ^ yを書き込みます。全て同じ長さが必要です。O(ビット数 / 64)。
        ## 領域確保・中間集合は不要で、dstにxやy自身を指定することもできます。
        ## AVX2経路の目安: 256ビットあたり論理演算1命令。ロード・ストアと末尾のマスクは別です。
        when compileOption("boundChecks"):
    ...
    proc andNotInto*(dst: var BitSetAvx2, x, y: BitSetAvx2) =
        ## 確保済みのdstへx & ~yを書き込みます。全て同じ長さが必要です。O(ビット数 / 64)。
        ## 領域確保・中間集合は不要で、dstにxやy自身を指定することもできます。
        ## AVX2経路の目安: 256ビットあたり論理演算1命令。ロード・ストアと末尾のマスクは別です。
        when compileOption("boundChecks"):
    ...
    proc xnorInto*(dst: var BitSetAvx2, x, y: BitSetAvx2) =
    ...
    template `&=~`*(x: var BitSetAvx2, y: BitSetAvx2) =
        ## xを差集合x & ~yに更新します。O(ビット数 / 64)。
        ## AVX2経路の目安: 256ビットあたりANDNOT 1命令。テンプレートの追加命令はありません。
        andNotAssign(x, y)
    ...
    proc `~`*(x: BitSetAvx2): BitSetAvx2 =
        ...
        ## AVX2経路の目安: 256ビットあたり反転 1命令（全ビット1とのXORなど）。
        result = initBitSet(x.size)
        if x.bits.len > 0:
    ...
    proc popcount*(x: BitSetAvx2): int =
        ## 立っているビットの個数を返します。
        ## AVX2経路の目安: 256ビットあたり約7命令（AND 2＋シフト1＋PSHUFB 2＋加算2）。最大16ベクトルごとにSAD＋加算の2命令、最後の4レーンの集約が別途必要です。
        if x.bits.len > 0:
    ...
    proc popcountrange*(x: BitSetAvx2, l, r: int): int =
    ...
    proc andpopcountrange*(x, y: BitSetAvx2, l, r: int): int =
    ...
    proc orpopcountrange*(x, y: BitSetAvx2, l, r: int): int =
    ...
    proc xorpopcountrange*(x, y: BitSetAvx2, l, r: int): int =
    ...
    proc `[]`*(bitset: BitSetAvx2, idx: Natural): bool {.backwardsIndex.} =
    ...
    proc `[]=`*(bitset: var BitSetAvx2, idx: Natural, x: bool) {.backwardsIndex.} =
    ...
    proc `[]=`*(bitset: var BitSetAvx2, idx: Natural, x: int) {.backwardsIndex.} =
    ...
    proc lexLess*(x, y: BitSetAvx2): bool {.inline.} =
