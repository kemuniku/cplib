...
data:
  ...
  code: "# https://github.com/tatyam-prime/SortedSet/blob/main/SortedMultiset.py\n\
    ...
    \ openArray[T] = []): SortedMultiSet[T] =\n        #Make a new SortedMultiset\
    \ from seq. / O(N) if sorted / O(N log N)\n        var v = @v\n        if not\
    \ isSorted(v):\n            v.sort()\n        var n = len(v)\n        var bucket_size\
    \ = int(ceil(sqrt(n/BUCKET_RATIO)))\n        var arr = collect(newseq): (for i\
    \ in 0..<bucket_size: v[(n*i div bucket_size) ..< (n*(i+1) div bucket_size)])\n\
    ...
    \   self.innerpop(b, i)\n        return true\n\n    proc lt*[T](self: SortedMultiSet[T],\
