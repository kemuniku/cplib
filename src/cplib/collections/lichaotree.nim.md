...
data:
  ...
  code: "import sequtils,algorithm\nimport cplib/utils/constants\nimport bitops\n\n\
    ...
    \ = newseqwith(2*lastnode,INF64)\n    result.lastnode = lastnode\n\nproc query(self:LiChaoTree,a,b,now:int)=\n\
    \    var a = a\n    var b = b\n    var now = now\n    var length = self.lastnode\
    \ shr fastLog2(now)\n    var i = now xor (1 shl fastLog2(now))\n    var l = i*length\n\
    \    var r = (i+1)*length\n    while true:\n        var m = (l+r) shr 1\n    \
    \    var left = self.X[l]*a+b < self.X[l]*self.A[now]+self.B[now]\n        var\
    \ right = self.X[r-1]*a+b < self.X[r-1]*self.A[now]+self.B[now]\n        var mid\
    \ = self.X[m]*a+b < self.X[m]*self.A[now]+self.B[now]\n\n        if left and right:\n\
    \            self.A[now] = a\n            self.B[now] = b\n            return\n\
  ...
  requiredBy: []
  timestamp: '2026-02-20 17:14:37+09:00'
