...
data:
  ...
  code: "## \u30B7\u30D5\u30C8\u3068OR\u3092\u878D\u5408\u3057\u3001\u5165\u529B\u3092\
    \u58CA\u3055\u306A\u3044\u65B9\u5411\u3067\u66F4\u65B0\u3057\u307E\u3059\u3002\
    \n{.emit: \"\"\"\n#include <immintrin.h>\n#include <stdint.h>\n#include <stddef.h>\n\
    __attribute__((target(\"avx2\"))) static void cplib_fuse_left_4(uint64_t *x, size_t\
    \ n, size_t k) {\nsize_t off = k >> 6; unsigned b = k & 63;\nconst __m128i s =\
    \ _mm_cvtsi32_si128(b), t = _mm_cvtsi32_si128(64-b);\nsize_t end = n;\nwhile (end\
    \ >= off + (b != 0) + 4) {\nsize_t j = end - 4;\n__m256i v = _mm256_loadu_si256((const\
    \ __m256i *)(x+(j-off)));\nif (b) v = _mm256_or_si256(_mm256_sll_epi64(v,s), _mm256_srl_epi64(_mm256_loadu_si256((const\
    \ __m256i *)(x+(j-off-1))),t));\n_mm256_storeu_si256((__m256i *)(x+(j)), _mm256_or_si256(_mm256_loadu_si256((const\
    \ __m256i *)(x+(j))),v));\nend -= 4;\n}\nwhile (end > off) { size_t j = --end;\
    \ uint64_t v = x[j-off] << b; if (b && j > off) v |= x[j-off-1] >> (64-b); x[j]\
    \ |= v; }\n}\n__attribute__((target(\"avx2\"))) static void cplib_fuse_right_4(uint64_t\
    \ *x, size_t n, size_t k) {\nsize_t off = k >> 6; unsigned b = k & 63;\nconst\
    ...
    \ __m256i *)(x+(j+off+1))),t));\n_mm256_storeu_si256((__m256i *)(x+(j)), _mm256_or_si256(_mm256_loadu_si256((const\
    \ __m256i *)(x+(j))),v));\nj += 4;\n}\nfor (; j < count; ++j) { uint64_t v = x[j+off]\
    \ >> b; if (b && j+off+1 < n) v |= x[j+off+1] << (64-b); x[j] |= v; }\n}\n__attribute__((target(\"\
    avx512f\"))) static void cplib_fuse_left_8(uint64_t *x, size_t n, size_t k) {\n\
    ...
    \ void *)(x+(j-off-1))),t));\n_mm512_storeu_si512((void *)(x+(j)), _mm512_or_si512(_mm512_loadu_si512((const\
    \ void *)(x+(j))),v));\nend -= 8;\n}\nwhile (end > off) { size_t j = --end; uint64_t\
    \ v = x[j-off] << b; if (b && j > off) v |= x[j-off-1] >> (64-b); x[j] |= v; }\n\
    }\n__attribute__((target(\"avx512f\"))) static void cplib_fuse_right_8(uint64_t\
    \ *x, size_t n, size_t k) {\nsize_t off = k >> 6; unsigned b = k & 63;\nconst\
    \ __m128i s = _mm_cvtsi32_si128(b), t = _mm_cvtsi32_si128(64-b);\nsize_t j = 0,\
    ...
    \ void *)(x+(j))),v));\nj += 8;\n}\nfor (; j < count; ++j) { uint64_t v = x[j+off]\
    \ >> b; if (b && j+off+1 < n) v |= x[j+off+1] << (64-b); x[j] |= v; }\n}\nstatic\
    \ void cplib_fuse_left(uint64_t *x, size_t n, size_t k) {\nif (__builtin_cpu_supports(\"\
    avx512f\")) cplib_fuse_left_8(x,n,k); else cplib_fuse_left_4(x,n,k);\n}\nstatic\
    \ void cplib_fuse_right(uint64_t *x, size_t n, size_t k) {\nif (__builtin_cpu_supports(\"\
    avx512f\")) cplib_fuse_right_8(x,n,k); else cplib_fuse_right_4(x,n,k);\n}\n\"\"\
    \".}\n\nproc fusedShiftLeft(x: ptr uint64, n, k: csize_t) {.importc: \"cplib_fuse_left\"\
    , nodecl.}\nproc fusedShiftRight(x: ptr uint64, n, k: csize_t) {.importc: \"cplib_fuse_right\"\
    , nodecl.}\n"
  dependsOn: []
  ...
  - verify/AI/bitset_avx512_add_test.nim
documentation_of: cplib/collections/private/bitset_avx512_fuse_shift.nim
layout: document
redirect_from:
- /library/cplib/collections/private/bitset_avx512_fuse_shift.nim
- /library/cplib/collections/private/bitset_avx512_fuse_shift.nim.html
