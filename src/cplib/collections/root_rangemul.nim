when not declared CPLIB_COLLECTIONS_ROOTRANGESUM:
    ...
    type RootRangeSum*[T] = ref object
        ...
        arr_inv : seq[T]
        e : T
    proc initrangesum*[T](v:seq[T],invv:seq[T],bsize:int = v.len.float.sqrt.int(),e:T=1):RootRangeSum[T]=
        var b = newseqwith((len(v)+bsize-1) div bsize,e)
        result = RootRangeSum[T](blocksize:bsize,length:len(v),arr:v,blockvalue:b,e:e,arr_inv:invv)
        for i in 0..<(len(v)):
            result.blockvalue[i div bsize] = result.blockvalue[i div bsize] * v[i]

    proc update*[T](self: RootRangeSum[T], idx: Natural, val: T,val_inv : T) =
        ## idxの要素をvalに変更します。
        assert idx < self.length
        self.blockvalue[idx div self.blocksize] = self.blockvalue[idx div self.blocksize] * val * self.arr_inv[idx]
        self.arr[idx] = val
        self.arr_inv[idx] = val_inv
