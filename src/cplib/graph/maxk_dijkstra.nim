when not declared CPLIB_GRAPH_MAXK_DIJKSTRA:
    ...
    declareMaxkDijkstra(maxk_dijkstra, int32, 0i32, INFi32)
    ...
    proc shortest_path*(G: DynamicGraph[int] or StaticGraph[int], start, goal, k: int, ZERO: int = 0, INF: int = INFL): tuple[path: seq[int], cost: int] =
