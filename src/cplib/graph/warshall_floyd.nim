when not declared CPLIB_GRAPH_WARSHALLFLOYD:
    ...
    proc warshall_floyd*(g: DynamicGraph[int] or StaticGraph[int] or UnWeightedGraph, zero: int = 0, inf: int = INF64): tuple[negative_cycle: bool, d: seq[seq[int]]] = warshall_floyd_impl(g, zero, inf)
