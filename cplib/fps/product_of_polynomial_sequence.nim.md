...
data:
  ...
  code: "when not declared CPLIB_FPS_PRODUCT_OF_POLYNOMIAL_SEQUENCE:\n    const CPLIB_FPS_PRODUCT_OF_POLYNOMIAL_SEQUENCE*\
    ...
    \ degreePrefix[middle] < target: low = middle + 1\n                else: high\
    \ = middle\n            result = low\n            if result > left + 1:\n    \
    \            let currentDifference = abs(\n                    (degreePrefix[result]\
    \ - degreePrefix[left]) -\n                    (degreePrefix[right] - degreePrefix[result]))\n\
    \                let previousDifference = abs(\n                    (degreePrefix[result\
    \ - 1] - degreePrefix[left]) -\n                    (degreePrefix[right] - degreePrefix[result\
    \ - 1]))\n                if previousDifference < currentDifference: result -=\
    \ 1\n\n        proc solve(left, right: int): seq[T] =\n            if left + 1\
    \ == right: return factors[left]\n            let middle = balancedMiddle(left,\
    \ right)\n            result = convolution(solve(left, middle), solve(middle,\
  ...
  - verify/AI/fps_test.nim
documentation_of: cplib/fps/product_of_polynomial_sequence.nim
layout: document
redirect_from:
- /library/cplib/fps/product_of_polynomial_sequence.nim
- /library/cplib/fps/product_of_polynomial_sequence.nim.html
